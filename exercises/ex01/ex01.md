...
```
  movl  $5, -4(%rbp)
```
saves the number "5" somewhere.

11. What happens if you turn on optimization using the flag `-O2`?

Uhh that line disappears, I don't know where it saves the number 5 now...
...
Without optimization, the same line `movl  $5, -4(%rbp)` exists.  With optimization, the line changes to `  movl  $5, %edx`.
...
Without optimization: ` movl  $5, -8(%rbp)`.

With optimization: `  movl  $6, %edx`.

What conclusions can you draw about how optimization works?

It seems like optimization consolidates code that doesn't need to exist, or removes middle-man code.
